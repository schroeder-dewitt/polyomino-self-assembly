...
__global__ void SearchSpaceKernel(unsigned char *g_ucGenomes,  unsigned char *g_ucGrids, float *g_ucFitnessLeft, float *g_ucFitnessBottom,  curandState *states)
{
    ...
    s_xGenomeSet.data.multi_d[r_xThreadInfo.BankId()].data.one_d[0] = 33; //40
    ...
    if(r_xThreadInfo.WarpId()==0){
        ...
        g_ucFitnessLeft[(blockIdx.y*m_fit_DimBlockX + blockIdx.x)*32+r_xThreadInfo.BankId()] = (float)  s_xAssembly.data.assembly_size[r_xThreadInfo.BankId()];//s_xAssembly.data.gravity[r_xThreadInfo.BankId()].x;
