...
__device__ void xCellGrid::Initialise(xThreadInfo __xThreadInfo,
	...
	for (int i = 0; i < m_fit_DimGridX; i++) {
        ...
        for(int i=0;i<myshare;i++){
                this->data.mix_d[__xThreadInfo.WarpId()*offset + i][__red][__xThreadInfo.BankId()].set_xCell(mEMPTY_CELL); 
        }
        if(__xThreadInfo.WarpId()==mBankSize-1){
                for(int i=0;i<offset;i++){
                        //this->data.one_d[one_d_off + mBankSize*myshare + i].set_xCell(mEMPTY_CELL);
                        this->data.mix_d[mBankSize*offset + i][__red][__xThreadInfo.BankId()].set_xCell(mEMPTY_CELL);
                }
        }
        //__syncthreads(); 
}
...
__global__ void SearchSpaceKernel(unsigned char *g_ucGenomes, float *g_ucFitnessValues, unsigned char *g_ucGrids, curandState *states)
{
    __shared__ xGenomeSet s_xGenomeSet;
    //__shared__ xEdgeSort s_xEdgeSort;
    __shared__ xAssembly s_xAssembly;
