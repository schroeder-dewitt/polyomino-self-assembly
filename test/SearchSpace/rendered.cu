...
__device__ bool xAssembly::Assemble_Movelist(xThreadInfo __xThreadInfo, xGenomeSet *__xGenomeSet) {
	...
	while (BUF.bNotTraversed()) {
		...
		//unsigned char DBGVAL = TmpAddPerm.WalkIndex;
                ...
                this->data.grid.set_xCell( __xThreadInfo, this->data.grid.xGetNeighbourCellCoords(X.x, X.y, (unsigned char) (a-1)).x, this->data.grid.xGetNeighbourCellCoords(X.x, X.y, (unsigned char) (a-1)).y, this->data.flags[__xThreadInfo.BankId()].get_ucRed(), 250);
