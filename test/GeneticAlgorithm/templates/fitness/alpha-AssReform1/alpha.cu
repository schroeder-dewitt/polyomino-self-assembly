...
__device__ bool xAssembly::Assemble_Movelist(xThreadInfo __xThreadInfo, xGenomeSet *__xGenomeSet) {
	...
	while (BUF.bNotTraversed()) {
		...
		//unsigned char DBGVAL = TmpAddPerm.WalkIndex;
                ...
                this->data.grid.set_xCell( __xThreadInfo, this->data.grid.xGetNeighbourCellCoords(X.x, X.y, (unsigned char) (index-1)).x, this->data.grid.xGetNeighbourCellCoords(X.x, X.y, (unsigned char) (index-1)).y, this->data.flags[__xThreadInfo.BankId()].get_ucRed(), 250);
				...
				if (TmpT.get_xCell() != mEMPTY_CELL) {
					if( (TmpT.get_xCell() != T.get_xCell()) && (T.get_xCell() != mEMPTY_CELL) ){
						...
						this->data.flags[__xThreadInfo.WarpId()].set_TrivialUND();
                                                /*BEGIN DEBUG*/
                                                this->data.grid.set_xCell( __xThreadInfo, 2, 0, this->data.flags[__xThreadInfo.BankId()].get_ucRed(), 15); //5
                                                /*END DEBUG*/
                                                return;
					}
			...
			if (this->data.flags[__xThreadInfo.BankId()].get_ucRed() > 0) {
				...
				if (TmpT.get_xCell() != T.get_xCell()) { //We have detected steric non-determinism!
					this->data.flags[__xThreadInfo.BankId()].set_StericUND(); //TEST
                                        /*START DEBUG*/
                                        this->data.grid.set_xCell( __xThreadInfo, 0, 0, this->data.flags[__xThreadInfo.BankId()].get_ucRed(), 31); //7
                                        /*END DEBUG*/
                                        return;
				}
			}
			...
			if (T.get_xCell() != mEMPTY_CELL) { 
				...
				if (!this->data.grid.set_xCell(	__xThreadInfo, X.x, X.y, this->data.flags[__xThreadInfo.BankId()].get_ucRed(), T.get_xCell())) {
					this->data.flags[__xThreadInfo.BankId()].set_UnboundUND();
                                        /*START DEBUG*/
                                        this->data.grid.set_xCell( __xThreadInfo, 1, 0, this->data.flags[__xThreadInfo.BankId()].get_ucRed(), 5);
                                        /*END DEBUG*/
                                        return;
				}
