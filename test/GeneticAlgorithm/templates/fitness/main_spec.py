...
def main():
    ...
    Source = env.get_template('./alpha_spec.cu') #Template( file(KernelFile).read() )
    # Render source code
    ...
    stop = drv.Event()
    start.record()
    Kernel(dest_h, grids_h, fitness_size_h, fitness_hash_h, curand_h, numpy.uint64(70149), block=(WarpSize*BankSize,1,1), grid=(BlockDimX,BlockDimY))
    stop.record()
    stop.synchronize()
