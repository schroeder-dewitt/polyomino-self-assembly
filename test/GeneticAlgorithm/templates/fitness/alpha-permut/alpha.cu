...
__device__ xFourPermutation::xFourPermutation(unsigned short __usPermIndex) {
    ...
    switch(__usPermIndex % 24){
        case 1: this->Perm = make_uchar4(1,2,3,4);
        case 2: this->Perm = make_uchar4(1,2,4,3);
        case 3: this->Perm = make_uchar4(1,3,2,4);
        case 4: this->Perm = make_uchar4(1,3,4,2);
        case 5: this->Perm = make_uchar4(1,4,2,3);
        case 6: this->Perm = make_uchar4(1,4,3,2);
        case 7: this->Perm = make_uchar4(2,1,3,4);
        case 8: this->Perm = make_uchar4(2,1,4,3);
        case 9: this->Perm = make_uchar4(2,3,1,4);
        case 10: this->Perm = make_uchar4(2,3,4,1);
        case 11: this->Perm = make_uchar4(2,4,1,3);
        case 12: this->Perm = make_uchar4(2,4,3,1);
        case 13: this->Perm = make_uchar4(3,2,1,4);
        case 14: this->Perm = make_uchar4(3,2,4,1);
        case 15: this->Perm = make_uchar4(3,1,2,4);
        case 16: this->Perm = make_uchar4(3,1,4,2);
        case 17: this->Perm = make_uchar4(3,4,2,1);
        case 18: this->Perm = make_uchar4(3,4,1,2);
        case 19: this->Perm = make_uchar4(4,2,3,1);
        case 20: this->Perm = make_uchar4(4,2,1,3);
        case 21: this->Perm = make_uchar4(4,3,2,1);
        case 22: this->Perm = make_uchar4(4,3,1,2);
        case 23: this->Perm = make_uchar4(4,1,2,3);
        case 0: this->Perm = make_uchar4(4,1,3,2);
    }
}
...
__device__ bool xAssembly::Assemble_Movelist(xThreadInfo __xThreadInfo, xGenomeSet *__xGenomeSet) {
	...
	while (BUF.bNotTraversed()) {
		...
		//unsigned char DBGVAL = TmpAddPerm.WalkIndex;
                this->data.movelist.bPush(__xThreadInfo, this->data.grid.xGetNeighbourCellCoords(X.x, X.y, (unsigned char) (a-1)));
	}
	...
	while ((this->data.movelist.get_sPos(__xThreadInfo) >= 0) && (!this->data.flags[__xThreadInfo.BankId()].get_bUNDCondition())) {
		...
		if (!this->data.flags[__xThreadInfo.BankId()].get_bUNDCondition() && T.get_xCell() != mEMPTY_CELL) {
			...
			if (!this->data.flags[__xThreadInfo.BankId()].get_bUNDCondition()) {
				...
				if (T.get_xCell() != mEMPTY_CELL) { //Now: Assemble tile!
...
__global__ void SearchSpaceKernel(unsigned char *g_ucGenomes,  unsigned char *g_ucGrids, float *g_ucFitnessLeft, float *g_ucFitnessBottom,  curandState *states)
{
    ...
    s_xGenomeSet.data.multi_d[r_xThreadInfo.BankId()].data.one_d[0] = 8; //33; //40
    s_xGenomeSet.data.multi_d[r_xThreadInfo.BankId()].data.one_d[1] = 16;
