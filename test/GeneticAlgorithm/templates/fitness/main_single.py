...
def main():
    ...
    env = Environment(loader=PackageLoader('main_single', './'))
    # Load source code from file
    Source = env.get_template('./alpha_single.cu') #Template( file(KernelFile).read() )
    # Render source code
...
#(histo[fitness_hash[j]][0], histo[fitness_hash[j]][1], histo[fitness_hash[j]][2]+1, histo[fitness_hash[j]][3])
        ...
        #DEBUG
        f = open("Single_tmp.s28", "w")
        for j in range(100): 
            print >>f, "Size: ", fitness_size[j+100], " Hash:", fitness_hash[j+100], " Genome:", dest[j*AlignedByteLengthGenome], "|",dest[j*AlignedByteLengthGenome+1]," | ",dest[j*AlignedByteLengthGenome+2], " | ", dest[j*AlignedByteLengthGenome+3]
            print >>f, grids[j+100]
        f.close()
        ...
        #WRITE TO FILE
        strng = ""
        filed = open("Single_"+str(time.time())+".s28", "w")
