...
/*    if(r_xThreadInfo.WarpId() < mAlignedByteLengthGenome){
    ...
    if(r_xThreadInfo.BankId()%3==0){
    s_xGenomeSet.data.multi_d[r_xThreadInfo.BankId()].data.one_d[0] = 5;//41; // 40; //41; //132; // 40; //33;//4; //33; //40
    s_xGenomeSet.data.multi_d[r_xThreadInfo.BankId()].data.one_d[1] = 0;
    } else if (r_xThreadInfo.BankId()%3==1) {
    s_xGenomeSet.data.multi_d[r_xThreadInfo.BankId()].data.one_d[0] = 40;
    s_xGenomeSet.data.multi_d[r_xThreadInfo.BankId()].data.one_d[1] = 0;
    } else if (r_xThreadInfo.BankId()%3==2){
    s_xGenomeSet.data.multi_d[r_xThreadInfo.BankId()].data.one_d[0] = 1;
...
*/
        g_ucGenomes[ ((blockIdx.y*m_fit_DimBlockX + blockIdx.x)*32+r_xThreadInfo.BankId())*mAlignedByteLengthGenome +0] = s_xAssembly.data.corner_upper[r_xThreadInfo.BankId()].x;
        g_ucGenomes[ ((blockIdx.y*m_fit_DimBlockX + blockIdx.x)*32+r_xThreadInfo.BankId())*mAlignedByteLengthGenome +1] = s_xAssembly.data.corner_upper[r_xThreadInfo.BankId()].y;
        g_ucGenomes[ ((blockIdx.y*m_fit_DimBlockX + blockIdx.x)*32+r_xThreadInfo.BankId())*mAlignedByteLengthGenome +2] = s_xAssembly.data.corner_lower[r_xThreadInfo.BankId()].x;
        g_ucGenomes[ ((blockIdx.y*m_fit_DimBlockX + blockIdx.x)*32+r_xThreadInfo.BankId())*mAlignedByteLengthGenome +3] = s_xAssembly.data.corner_lower[r_xThreadInfo.BankId()].y;
