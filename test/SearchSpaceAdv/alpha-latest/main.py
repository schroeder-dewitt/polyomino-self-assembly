...
#            if (fitness_hash[j]!=33) and (fitness_hash[j]!=44) and (fitness_hash[j]!=22):
            if fitness_hash[j] in histo: 
                histo[fitness_hash[j]] = (grids[j], dest[j*AlignedByteLengthGenome]+dest[j*AlignedByteLengthGenome+1]+dest[j*AlignedByteLengthGenome+2]+dest[j*AlignedByteLengthGenome+3], histo[fitness_hash[j]][2]+1, fitness_size[j])
#(histo[fitness_hash[j]][0], histo[fitness_hash[j]][1], histo[fitness_hash[j]][2]+1, histo[fitness_hash[j]][3])
            else:
                histo[fitness_hash[j]] = (grids[j], dest[j*AlignedByteLengthGenome]+dest[j*AlignedByteLengthGenome+1]+dest[j*AlignedByteLengthGenome+2]+dest[j*AlignedByteLengthGenome+3], 1, fitness_size[j])

        #filed = open("S28_"+str(time.time())+"."+str(i)+".s28", "w")
        #print >>filed, histo
        #filed.close()
        print "Filing..."
        #WRITE TO FILE
    ...
    for i in range(grids.shape[0]):
        if fitness_hash[i] in histo:
            ...
            histo[fitness_hash[i]] = (grids[i], dest[i*AlignedByteLengthGenome]+dest[i*AlignedByteLengthGenome+1]+dest[i*AlignedByteLengthGenome+2]+dest[i*AlignedByteLengthGenome+3], 1, fitness_size[i])
