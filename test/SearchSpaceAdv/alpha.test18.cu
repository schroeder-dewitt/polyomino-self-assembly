...
__device__ float xAssembly::fEvaluateFitness(xThreadInfo __xThreadInfo){
	...
	//this->data.grid.set_xCell( __xThreadInfo, 0, 0, this->data.flags[__xThreadInfo.BankId()].get_ucRed(),  this->data.corner_upper[__xThreadInfo.BankId()].x);
		...
		/*union {
                ...
                /*int minhash = this->data.hash[__xThreadInfo.BankId()].x;
                if(this->data.hash[__xThreadInfo.BankId()].y < minhash){
                    minhash = this->data.hash[__xThreadInfo.BankId()].y;
                }
                if(this->data.hash[__xThreadInfo.BankId()].z < minhash){
                    minhash = this->data.hash[__xThreadInfo.BankId()].z;
                }
                if(this->data.hash[__xThreadInfo.BankId()].w < minhash){
                    minhash = this->data.hash[__xThreadInfo.BankId()].w;
                }
                
                this->data.hash[__xThreadInfo.BankId()].x = minhash; */
			...
			jenkins_add(conv.chars[i], newhash);	
                ...
                */
                this->data.fin_hash[__xThreadInfo.BankId()] = 0;
                this->data.fin_hash[__xThreadInfo.BankId()] += this->data.hash[__xThreadInfo.BankId()].x;
                this->data.fin_hash[__xThreadInfo.BankId()] += this->data.hash[__xThreadInfo.BankId()].y;
                this->data.fin_hash[__xThreadInfo.BankId()] += this->data.hash[__xThreadInfo.BankId()].z;
                this->data.fin_hash[__xThreadInfo.BankId()] += this->data.hash[__xThreadInfo.BankId()].w;
        }
        ...
        __syncthreads();
        this->data.hash[__xThreadInfo.BankId()].x = this->data.assembly_size[__xThreadInfo.BankId()];
...
//    }
	...
	g_ucFitnessSize[(blockIdx.y*m_fit_DimBlockX + blockIdx.x)*32+r_xThreadInfo.BankId()] = s_xAssembly.data.assembly_size[r_xThreadInfo.BankId()];
        g_ucFitnessHash[(blockIdx.y*m_fit_DimBlockX + blockIdx.x)*32+r_xThreadInfo.BankId()] = (int) s_xAssembly.data.fin_hash[r_xThreadInfo.BankId()];
    }
