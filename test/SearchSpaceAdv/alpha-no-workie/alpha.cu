...
/*	if ((__x < m_fit_DimGridX) && (__y < m_fit_DimGridY)) {
        ...
        /*if( (__x == (m_fit_DimGridX - 1) ) || (__y == (m_fit_DimGridY - 1) ) || (__x == 0) || (__y == 0) ){ // On border
            return this->data.multi_d[__x][__y][__red][__xThreadInfo.BankId()];
        }*/
...
__device__ bool xCellGrid::set_xCell(xThreadInfo __xThreadInfo, unsigned char __x, unsigned char __y, unsigned char __red, unsigned char __val) {
	/*if ((__x < m_fit_DimGridX - 1) && (__y < m_fit_DimGridY - 1) && (__x !=0) && (__y != 0)) {
        ...
        if ( (__x <= m_fit_DimGridX-1) && (__x >= 0) && (__y <= m_fit_DimGridY-1) && (__y >= 0) ) { // In grid
            this->data.multi_d[__x][__y][__red][__xThreadInfo.BankId()].set_xCell(__val);
        } 
		...
		if (TmpFlag) {
			//for (int i = 0; (i < m_fit_NrRedundancyAssemblies) && (!this->data.flags[__xThreadInfo.BankId()].get_bUNDCondition()); i++) {
                        for (int i = 0; (i < m_fit_NrRedundancyAssemblies); i++) {
                                //this->data.flags[__xThreadInfo.BankId()].set_Red(i);
                                //__syncthreads();
                                if(!this->data.flags[__xThreadInfo.BankId()].get_bUNDCondition()){
					this->Initialise(__xThreadInfo, __xGenomeSet); //Empty out assembly grid at red
                                ...
                                //if (i==1) return;
                                //__syncthreads();
                                //this->Initialise(__xThreadInfo, __xGenomeSet);
                                //return;
                                //__syncthreads();   
                                //return;
                                //__syncthreads(); //TEST
...
__device__ bool xAssembly::Assemble_Movelist(xThreadInfo __xThreadInfo, xGenomeSet *__xGenomeSet) {
	...
	this->data.grid.set_xCell(__xThreadInfo, m_fit_DimGridX / 2, m_fit_DimGridY / 2, this->data.flags[__xThreadInfo.BankId()].get_ucRed()% m_fit_NrRedundancyGridDepth, 0);
	...
	while (BUF.bNotTraversed()) {
		...
		//unsigned char DBGVAL = TmpAddPerm.WalkIndex;
                ...
                this->data.grid.set_xCell( __xThreadInfo, this->data.grid.xGetNeighbourCellCoords(X.x, X.y, (unsigned char) (index)).x, this->data.grid.xGetNeighbourCellCoords(X.x, X.y, (unsigned char) (index)).y, this->data.flags[__xThreadInfo.BankId()].get_ucRed()% m_fit_NrRedundancyGridDepth, mEMPTY_CELL_ML);
	...
	xCell T, TmpT; // T(Mirr(E_X, N(E_X)) be potential bonding tiles
 	...
 	while (this->data.movelist.get_sPos(__xThreadInfo) > 0) {
		...
		for (int E_X = 0; E_X < mNrTileOrientations; E_X++) {
			...
			N = this->data.grid.xGetNeighbourCell(__xThreadInfo, X.x, X.y, this->data.flags[__xThreadInfo.BankId()].get_ucRed()% m_fit_NrRedundancyGridDepth, (unsigned char) E_X);
		...
		if (!this->data.flags[__xThreadInfo.BankId()].get_bUNDCondition() && T.get_xCell() != mEMPTY_CELL) {
			...
			//	TmpT2 = this->data.grid.get_xCell(__xThreadInfo, X.x, X.y, (this->data.flags[__xThreadInfo.BankId()].get_ucRed() - 1) % m_fit_NrRedundancyGridDepth);
			...
			if (T.get_xCell() != mEMPTY_CELL) { 
				...
				while (TmpAddPerm.bNotTraversed()) {
					...
					//For all n(E_X)
					N = this->data.grid.xGetNeighbourCell(__xThreadInfo, X.x, X.y, this->data.flags[__xThreadInfo.BankId()].get_ucRed()% m_fit_NrRedundancyGridDepth, (unsigned char) index2);
					...
					if (N.get_xCell() == mEMPTY_CELL) {
						...
						this->data.movelist.bPush(__xThreadInfo, this->data.grid.xGetNeighbourCellCoords(X.x, X.y, (unsigned char) index2)); 
                                                ...
                                                //return;
                                                //} //DEBUG
                                                this->data.grid.set_xCell( __xThreadInfo, this->data.grid.xGetNeighbourCellCoords(X.x, X.y, (unsigned char) (index2)).x, this->data.grid.xGetNeighbourCellCoords(X.x, X.y, (unsigned char) (index2)).y, this->data.flags[__xThreadInfo.BankId()].get_ucRed() % m_fit_NrRedundancyGridDepth, mEMPTY_CELL_ML);
...
__global__ void SearchSpaceKernel(unsigned char *g_ucGenomes,  unsigned char *g_ucGrids, float *g_ucFitnessLeft, float *g_ucFitnessBottom,  curandState *states)
{
    ...
    s_xGenomeSet.data.multi_d[r_xThreadInfo.BankId()].data.one_d[0] = 40; //41; //132; // 40; //33;//4; //33; //40
    s_xGenomeSet.data.multi_d[r_xThreadInfo.BankId()].data.one_d[1] = 0; //192; //6; //162; //160 //128; //0;//32;//16;
