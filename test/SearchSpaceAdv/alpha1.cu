...
struct xLifoList {
	...
	__device__ bool bPush(T __xEntry, T* __xStorage, unsigned short __uiMaxLength);
...
template<class T>
__device__ bool xLifoList<T>::bPush(T __xEntry, T* __xStorage,
		unsigned short __uiMaxLength) {
	this->data.pos++;
	if (this->data.pos < __uiMaxLength) {
		__xStorage[this->data.pos] = __xEntry;
		return true;
	} else {
		this->data.pos--;
		return false;
...
template<class T>
__device__ T xLifoList<T>::xPop(T* __xStorage) {
	if (this->data.pos < 0) {
		...
		this->data.pos--;
		return __xStorage[this->data.pos + 1];
	}
}
...
__device__ bool xMoveList<T>::bPush(xThreadInfo __xThreadInfo, T __xEntry) {
	this->list.multi_d[__xThreadInfo.BankId()].bPush(__xEntry, this->storage.multi_d[__xThreadInfo.BankId()], sizeof(this->storage) / sizeof(T));
	return true;
}
